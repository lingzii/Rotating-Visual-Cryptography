...
def main():
    # 輸入檔名以讀檔
    file = "mcu.png"  # input('Input the file name: ')
    ...
    # 建立噪圖 (隨機生成)
    noise = np.random.uniform(0, 255, image.shape).astype('uint8')
    ...
    secretLayer = cv2.bitwise_and(imageLayer, noiseLayer)
    cv2.imwrite('output.png', secretLayer)
